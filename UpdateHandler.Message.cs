...
{
     private readonly Dictionary<long, int> locationRequestMessages = new();
    private async Task HandleMessageUpdateAsync(ITelegramBotClient botClient, Message message, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Received Message from {username}", username);

        if(locationRequestMessages.ContainsKey(message.Chat.Id))
        {
            await RemoveMessageAsync(botClient, message.Chat.Id, locationRequestMessages[message.Chat.Id], cancellationToken: cancellationToken);
        ...
        if(message.Location is not null)    // location sent
        {
            try
            {
                var weatherText = await _weatherService.GetWeatherTextAsync(
